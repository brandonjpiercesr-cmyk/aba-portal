...
export default function EmailPage() {
  ...
  async function traceEmail(email) {
    ...
    const d = await fetch(`/api/email/trace?subject=${encodeURIComponent(email.subject)}&id=${email.id}`).then(r=>r.json());
  ...
  return (<div>
    <PageTitle right={<span className="text-dim text-xs">{filtered.length.toLocaleString()} emails</span>}>Email Audit</PageTitle>
    <div className="flex gap-2 mb-4 flex-wrap">
      ...
      <select className="w-32" value={hours} onChange={e=>setHours(e.target.value)}>
        <option value="6">6h</option><option value="24">24h</option><option value="72">3 days</option><option value="168">7 days</option>
      </select>
    ...
    <Card>{loading?<Loading/>:(
      ...
      <tbody>{filtered.map((e,i)=>(
        <tr key={i} data-aba-ctx={JSON.stringify({type:'email',label:e.subject,data:{from:e.from,to:e.to,subject:e.subject,date:e.date,grant:e.grant}})}>
          ...
          <td className="font-medium text-white max-w-[250px] truncate">{e.subject}</td>
        ...
        <div className="border-t border-border pt-3">
          ...
          {tracing?<Loading text="Tracing code path..."/>:trace?(
            <div className="space-y-3">
              ...
              <div className="text-dim text-[10px]">{trace.total} total trace records found</div>
            </div>
          ):null}
        </div>
      </div>)}
